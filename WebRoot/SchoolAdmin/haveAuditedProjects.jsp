...
<script>
	...
	ProjectInfor.prototype.showDetailProject = function(pro_id){//显示课题的详细信息内容(根据课题编号查询课题详细信息内容)
		return function(){
			var url = ifIE ? "../ProjectAuditServlet?type=project_audit_infor&project_id="+pro_id : "ProjectAuditServlet?type=project_audit_infor&project_id="+pro_id;
	...
	PagingFrame.prototype.createView = function(){
		...
		DWRAuditPaging.getPageNumberBySchoolId($("auditer_id").value, 1, null, function(pageParams){
	...
	PageProject.prototype.createPage = function(){
		try{
			if(this.pro_infor.length > 0){
				...
				promptWin.createConfirmView("<span style='color:red;'>当前不存在您已审核过的课题！</span>" , "确定");	
			}
		}catch(error){
			alert(error);
		}
...
<div class="project_body">
	<div id="projects_out">
		...
		<input type="hidden" value="<%=auditer_id %>" id="auditer_id"/>
	</div>
