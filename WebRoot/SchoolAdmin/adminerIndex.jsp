...
function selectButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("设计毕业课题" , "main.jsp", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("学校毕业课题" , "http://www.baidu.com", "main_body", obj.button).createLink();
			var link2 = new ChildLinkObj("我的课题" , "http://www.baidu.com", "main_body", obj.button).createLink();
			var link3 = new ChildLinkObj("回收站" , "http://www.baidu.com", "main_body", obj.button).createLink();
...
function teamButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			//这个数据来源是使用Ajax获取的
			var link0 = new ChildLinkObj("zero团队" , "main.jsp", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("zhaoyu" , "http://www.baidu.com", "main_body", obj.button).createLink();
			var link2 = new ChildLinkObj("Good团队" , "http://www.baidu.com", "main_body", obj.button).createLink();
...
function stageButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var url = "ProjectManagerServlet?type=prostage_title";
			showStageTitles(url);
			...
			var linkcreate = false;
			var tTimer = setInterval(function(){
				if(stageTitles != null){
					frame.removeChild(frame.childNodes[0]);//删除等待对象子元素
					for(var i=0; i<stageTitles.length; i++){
						
						var link = new ChildLinkObj(stageTitles[i].stage_name , "main.jsp", "main_body", obj.button).createLink();
						frame.appendChild(link);
					}
					
					clearInterval(tTimer);//清除事件对象
				}else{
					if(!linkcreate){
...
function userButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("用户信息查看" , "UserManagerServlet?type=adminer_user_infor", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("用户信息更改" , "UserManagerServlet?type=come_update_adminer", "main_body", obj.button).createLink();
		...
		<div id="header_menu_frame">   
		    <div id="header_menu">
		        <ul>
		            ...
		            <li><a href="stuSelectProject.jsp" id="schoolInfor_page" class="head_link_out" target="main_body">学校信息管理</a></li>
		            <li><a href="schoolLogin.jsp" id="adminManage_page" class="head_link_out" target="main_body">学校用户管理</a></li>
		            <li><a href="#" id="STManage_page" class="head_link_out" target="main_body">学生教师管理</a></li>
		            <li><a href="#" id="schoolParams_page" class="head_link_out" target="main_body">学校参数设置</a></li>
		            <li><a href="#" id="userinfor_page" class="head_link_out" target="main_body">用户基本信息</a></li>
		        </ul>  	
