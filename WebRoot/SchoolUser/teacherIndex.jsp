...
function selectButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("设计毕业课题" , "ProjectManagerServlet?type=enter_projectPage", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("学校毕业课题" , "ProjectManagerServlet?type=enter_schoolPros", "main_body", obj.button).createLink();
			var link2 = new ChildLinkObj("我的课题" , "ProjectManagerServlet?type=enter_teacherPros", "main_body", obj.button).createLink();
			var link3 = new ChildLinkObj("回收站" , "SchoolUser/recycleProjects.jsp", "main_body", obj.button).createLink();
			
			frame.appendChild(link0);
			frame.appendChild(link1);
			frame.appendChild(link2);
			frame.appendChild(link3);
...
function teamButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			/* 设置一个团队信息获取异步访问 */
			var url = "TeamManageServlet?type=query_guide_teams";
			var teamRequest = newXMLHttpRequest();
			teamRequest.open("post" , url, true);
			teamRequest.onreadystatechange = setGuideTeams(teamRequest , frame, obj);//异步Ajax获取阶段名称数据
...
function setGuideTeams(teamRequest , frame, obj){//设置阶段名称信息
	return function(){
		if(teamRequest.readyState == 1 || teamRequest.readyState == 2 || teamRequest.readyState == 3){
			...
			if(teamRequest.status == 200){
				var guideTeams = eval(teamRequest.responseText);
				...
				if(guideTeams.length == 0){
					var loading = document.createElement("div");
					loading.className = "loading";
					loading.innerHTML = "<span style='color:red'>未指导团队...</span>";
					frame.appendChild(loading);
				}else{
					for(var i=0; i<guideTeams.length; i++){
						if(i == 0){
							$("teams_page").href = "TeamManageServlet?type=query_guide_teamInfor&teamerId="+guideTeams[i].teamId;
						}
						var link = new ChildLinkObj(guideTeams[i].teamName , "TeamManageServlet?type=query_guide_teamInfor&teamerId="+guideTeams[i].teamId, "main_body", obj.button).createLink();
...
function stageButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var url = "StageManageServlet?type=query_guide_stages";
			...
			if(stageRequest.status == 200){
				...
				if(stageObjs != null){
					...
					for(var i=0; i<stageObjs.length; i++){
						if(i == 0){
							$("audit_page").href = "StageManageServlet?type=query_stage_guide&stageId="+stageObjs[i].stageId+"&teamId="+stageObjs[i].teamId;
						}
...
function userButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("用户信息查看" , "UserManageServlet?type=teacher_user_infor", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("用户信息更改" , "UserManageServlet?type=come_update_teacher", "main_body", obj.button).createLink();
...
fillUserInfor.prototype.createView = function(){
	if(!this.ifDetail){
		...
		this.iframeObj.src = "SchoolUser/teacherLogon.jsp";
		
		var margin_Left = (document.documentElement.clientWidth - 600)/2;
		var title = new Window_titleview(null , null, new Window_title("填写教师用户详细信息(<span style='color:red'>*</span>为必填项)"), null, null, false, null, false, null, false);//设置标题信息
