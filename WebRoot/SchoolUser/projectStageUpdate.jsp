...
<%
	...
	String projectID = (String)request.getAttribute("project_id");
	int[] params = (int[])request.getAttribute("proStageParams");
%>
<html>
<head>
...
function init(){
	...
	stageParamsObj.initQuery($("stageUpTime").value , $("stageUpLimit").value, $("stageFlLimit").value);
	...
	stageFrameObj.setUpdateStageInfor();
	
	var stageSubmit = new projectStageSaveObj($("project_stage_update"));
...
projectStageParamObj.prototype.initQuery = function(stageUpTime , stageUpLimit, stageFlLimit){//去除这个学校参数获取的操作
	this.stageUpTime = stageUpTime;
	this.stageUpLimit = stageUpLimit;
	this.stageFlLimit = stageFlLimit;
	
	var forms = document.getElementsByTagName("form");
	//为当前已经存在的表单中的stage_timeLimit设置值
	for(var n=0;n<forms.length;n++){
		for(var i=0;i<this.stageUpTime;i++){
	...
	stageHelpFrame = new stageParamsFrame($("project_stage_params")).createView(this);
};
...
projectStageFrameObj.prototype.setUpdateStageInfor = function(){//设置一个用于转载需要更改的课题阶段数据内容(使用DWR操作)
	ProjectStageDeal.queryProjectStages($("projectID").value , function(stageInfors){//实现数据转载到页面中DWR框架
		if(stageInfors.length > 0){
			for(var i=0;i<stageInfors.length;i++){
				stageFrameObj.stageFrame.style.height = stageFrameObj.stageFrame.offsetHeight + 200+"px";
				//对于课题阶段的更改需要再添加一个原始数据的转载操作（在createWindow是添加一个原始数据对象）
				new StageCreateWindow(stageFrameObj.stageFrame , stageFrameObj.projectStageArray.length, stageFrameObj.showStageArray.length*200).createWindow(stageInfors[i]);
				stageFrameObj.resetStageLocation();
			}
		}else{
			//new StageCreateWindow(stageFrameObj.stageFrame , 0, 0).createWindow();	
		}
	});
...
StageCreateWindow.prototype.createWindow = function(stageInfor){
	...
	projectStage.style.position = "relative";
   	...
   	if(stageInfor == null || undefined){
   		...
   		this.stageForm.innerHTML = "<table width='100%' border='0'>"
	  							+"<tr><td width='25%' align='right'>阶段名称：</td>"
	    						+"<td width='36%'><div class='input_frame'><input type='text' name='stage_name' value='"+stageInfor.stageName+"' maxlength='20'/></div></td>"
	   							+"<td width='13%' align='right'>时间限定：</td>"
	    						+"<td width='26%'><select name='stage_timeLimit'></select><span style='margin-left:10px;'>天</span></td></tr>"
	  							+"<tr><td align='right'>阶段要求：</td><td colspan='3' rowspan='4'>"
	    						+"<div class='textarea_frame'><textarea name='stage_demand'>"+stageInfor.stageDemand+"</textarea></div></td></tr>"
	  							+"<tr><td align='right'>&nbsp;</td></tr>"
	  							+"<tr><td align='right'>&nbsp;</td></tr>"
	  							+"<tr><td align='right'>&nbsp;</td></tr>"
	...
	stage_demand.setTextInfor(stageInfor == null || undefined ? "" : stageInfor.stageDemand);
	...
	//设置原始的限定时间
	this.setStageTimeLimit(this.stageForm.elements["stage_timeLimit"] , stageInfor == null || undefined ? null : stageInfor.timeLimit);
...
StageCreateWindow.prototype.setStageTimeLimit = function(stageTimeObj , timelimit){//设置课题阶段的时间限定
	if(stageParamsObj != null){
		if(timelimit == null || undefined){
			...
			for(var i=0;i<stageParamsObj.stageUpTime;i++){
				...
				timelimit == i+1 ? stageTimeObj.options[i].selected = true : "";
			}
		}
	}
};
...
projectStageSaveObj.prototype.clickEvent = function(saveObj){//这个是用于处理对于需要提交的课题阶段的内容的格式等的检验操作
	return function(){
		if(stageFrameObj.existError() && stageParamsObj.checkStageParams() && !saveObj.submitEvent){
			...
			stageRequest.open("post" , "ProjectManagerServlet?type=update_ProjectStage&project_id="+$("projectID").value+"&stageJSON="+urlJSON, true);
		...
		if(stageRequest.readyState == 1 || stageRequest.readyState == 2 || stageRequest.readyState == 3){
			...
			if(stageRequest.status == 200){
				...
				if(result == "true"){//成功提交
					...
					saveObj.promptWin.createConfirmView("<span style='color:green;'>你提交的课题阶段信息已更改成功！</span>" , "确定");
				}else{
...
projectStageSaveObj.prototype.submitSuccess = function(saveObj){
	return function(){
		...
		var url = "ProjectManagerServlet?type=query_detail_project&project_id="+$("projectID").value;
...
<input type="hidden" id="stageUpTime" value="<%=params[0]%>"/>
<input type="hidden" id="stageUpLimit" value="<%=params[1]%>"/>
<input type="hidden" id="stageFlLimit" value="<%=params[2]%>"/>
<div id="project_stage_frame">
	...
	<div id="project_stage_Operate">
		...
		<input type="button" id="project_stage_update" value="阶段更改"></input>
	</div>
</div>
</body>
</html>
