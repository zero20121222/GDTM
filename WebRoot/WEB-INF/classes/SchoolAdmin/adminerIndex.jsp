...
function initlizeIframe(){
	...
	iframeLinkObj = new IframeLinksObj("Admin");//创建一个页面管理对象
...
IframeLinksObj.prototype.createCookie = function(userType){//创建一个用于保存改用户对象浏览改页面的URL路径以及Button的键值对关系表
	...
	cookie.setCookie(userType , this.toString(), 1);
};
...
function createIndexList(){
	...
	/* 其实可以通过实现一个通用类创建 */
	var select_obj = new ButtonObj($("schoolInfor_page"));
	select_obj.butOverEvent = schoolButtonOver;//复写ButtonObj的butOverEvent事件
	select_obj.createButton();
	
	var team_obj = new ButtonObj($("adminManage_page"));
	team_obj.butOverEvent = userMButtonOver;
	team_obj.createButton();

    var stm_obj = new ButtonObj($("STManage_page"));
    stm_obj.butOverEvent = STMButtonOver;
    stm_obj.createButton();

    var param_obj = new ButtonObj($("schoolParams_page"));
    param_obj.butOverEvent = paramButtonOver;
    param_obj.createButton();
...
function schoolButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("学校信息" , "SchoolManageServlet?type=query_school_info", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("院系信息" , "SchoolManageServlet?type=query_school_colleges", "main_body", obj.button).createLink();
			var link2 = new ChildLinkObj("公告信息" , "SchoolManageServlet?type=query_school_news", "main_body", obj.button).createLink();
...
function userMButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			//这个数据来源是使用Ajax获取的
			var link0 = new ChildLinkObj("管理员信息" , "SchoolManageServlet?type=query_school_auditors", "main_body", obj.button).createLink();
...
function STMButtonOver(obj){
    return function(){
        ...
        if(obj.frameout == null){
            ...
            var link0 = new ChildLinkObj("学生&教师信息" , "SchoolManageServlet?type=query_school_users", "main_body", obj.button).createLink();
            var link1 = new ChildLinkObj("导入人员文件" , "SchoolManageServlet?type=query_school_files", "main_body", obj.button).createLink();
...
function paramButtonOver(obj){
    return function(){
        ...
        if(obj.frameout == null){
            ...
            var link0 = new ChildLinkObj("学校参数设置" , "SchoolManageServlet?type=query_school_params", "main_body", obj.button).createLink();
...
function userButtonOver(obj){
	return function(){
		...
		if(obj.frameout == null){
			...
			var link0 = new ChildLinkObj("用户信息查看" , "UserManageServlet?type=adminer_user_infor", "main_body", obj.button).createLink();
			var link1 = new ChildLinkObj("用户信息更改" , "UserManageServlet?type=come_update_adminer", "main_body", obj.button).createLink();
...
fillUserInfor.prototype.createView = function(){
	if(!this.ifDetail){
		...
		this.iframeObj.src = "SchoolAdmin/adminerLogon.jsp";
		...
		<div id="header_menu_frame">   
		    <div id="header_menu">
		        <ul>
		            ...
		            <li><a href="SchoolManageServlet?type=query_school_info" id="schoolInfor_page" class="head_link_out" target="main_body">学校信息管理</a></li>
		            <li><a href="SchoolManageServlet?type=query_school_auditors" id="adminManage_page" class="head_link_out" target="main_body">学校用户管理</a></li>
		            <li><a href="SchoolManageServlet?type=query_school_users" id="STManage_page" class="head_link_out" target="main_body">学生教师管理</a></li>
		            <li><a href="SchoolManageServlet?type=query_school_params" id="schoolParams_page" class="head_link_out" target="main_body">学校参数设置</a></li>
		            <li><a href="UserManageServlet?type=adminer_user_infor" id="userinfor_page" class="head_link_out" target="main_body">用户基本信息</a></li>
		        </ul>  	
