...
<script type="text/javascript" src="dwr/interface/DWRAuditPaging.js"></script>
<style>
	*{
		margin: 0;
		padding: 0;
...
<script>
	...
	ProjectInfor.prototype.showDetailProject = function(pro_id){//显示课题的详细信息内容(根据课题编号查询课题详细信息内容)
		return function(){
			var url = ifIE ? "../ProjectAuditServlet?type=enter_project_audit&project_id="+pro_id : "ProjectAuditServlet?type=enter_project_audit&project_id="+pro_id;
	...
	function PagingFrame(findButton){
		...
		this.queryProStage = "W";//需要查询的课题的状态类型
	}
	PagingFrame.prototype.createView = function(){
		//先获取分页参数信息然后再显示(这个是一个默认的分页对象-》》》这个后期将会对于搜索方式更改为学校审核通过的课题信息.
		DWRAuditPaging.getPageNumberBySchoolId($("auditer_id").value, 1, "W", function(pageParams){
	...
	PagingFrame.prototype.changePaging = function(queryFun , queryIndex){//实现根据不同的搜索方式实现分页queryIndex:查询索引
		switch(queryFun){
			case 1:{
				//这个是获取当前页面的分页数目
				DWRAuditPaging.getPageNumberBySchoolId($("auditer_id").value, $("school_id").value , this.queryProStage, function(pageParams){
			...
			case 2:{
				DWRAuditPaging.getPageNumberByCreateName($("auditer_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
			...
			case 3:{
				DWRAuditPaging.getPageNumberByCollege($("auditer_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 4:{
				DWRAuditPaging.getPageNumberByProjectName($("auditer_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 5:{
				DWRAuditPaging.getPageNumberByPartNum($("auditer_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 6:{
				DWRAuditPaging.getPageNumberByHardNum($("auditer_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
	...
	PagingFrame.prototype.closeWinodw = function(parentObj){//重写关闭事件
		return function(){  
			...
			pagingFrameObj.queryProStage = "W";
		};
	};
	...
	PageProject.prototype.createPage = function(){
		try{
			if(this.pro_infor.length > 0){
				...
				promptWin.createConfirmView("<span style='color:red;'>当前不存在需要审核的课题！</span>" , "确定");	
			}
		}catch(error){
			alert(error);
		}
	...
	function PageSlideObj(pageIndex , allPageNum){
		...
		this.pagingProStage = "W";//当前需要查询的页面的课题状态类型（默认为S）
	}
	...
	PageSlideObj.prototype.findProjects = function(pageNum , page){/* 查找页面对象信息使用DWR技术调用后台数据信息 */
		...
		switch(this.pagingFunType){
			case 1:{
				DWRAuditPaging.pagingBySchoolId(pageNum-1 , $("auditer_id").value, $("school_id").value, this.pagingProStage, function(pageObj){
			...
			case 2:{
				DWRAuditPaging.pagingByCreateName(pageNum-1 , $("auditer_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 3:{
				DWRAuditPaging.pagingByCollege(pageNum-1 , $("auditer_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 4:{
				DWRAuditPaging.pagingByProjectName(pageNum-1 , $("auditer_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 5:{
				DWRAuditPaging.pagingByPartNum(pageNum-1 , $("auditer_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 6:{
				DWRAuditPaging.pagingByHardNum(pageNum-1 , $("auditer_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
...
<div class="project_body">
	<div id="projects_out">
		...
		<input type="hidden" value="${user_id }" id="auditer_id"/>
	</div>
