...
<style>
	*{
	...
	.add_to_dustbin{
		...
		float:left;
		cursor:pointer;
		background:url(images/delete_project.png);
	}
	.delete_Frame{
		width:400px;
		height:150px;
...
<script>
	...
	ProjectInfor.prototype.packagePro = function(){//将对象封装成为div对象
		...
		addEvent(pro_select , "click", this.showDetailProject(this.pro_id , this.pro_stage));
		
		//在这里添加一个对于教师课题的更改操作的button对象(更改信息的设置存放到显示详细内容的页面中去吧)
		/*
		var pro_update = document.createElement("div");
		pro_update.className = "pro_update";
		pro_update.title = "更改信息";
		addEvent(pro_update , "click", this.updateProject(this.pro_id));
		*/
	...
	ProjectInfor.prototype.showDetailProject = function(pro_id , pro_stage){//显示课题的详细信息内容(根据课题编号查询课题详细信息内容)
		return function(){
			var url = null;
			if(pro_stage=="S" || pro_stage=="D"){
				//显示课题详细信息（可以更改）
				url = ifIE ? "../ProjectManagerServlet?type=query_detail_project&project_id="+pro_id : "ProjectManagerServlet?type=query_detail_project&project_id="+pro_id;
			}else{
				//显示教师详细审核信息内容(显示全部审核信息)
				url = ifIE ? "../ProjectManagerServlet?type=query_project_audit&project_id="+pro_id : "ProjectManagerServlet?type=query_project_audit&project_id="+pro_id;
			}
	...
	ProjectInfor.prototype.createDeleteFrame = function(){
		...
		var url = this.pro_picture == "null" ? "images/upload_file.png" : "ProjectInfor/"+this.pro_schoolId+"/"+this.pro_userid+"/"+this.pro_picture;
		var pro_image = document.createElement("img");
		pro_image.style.width = "50px";
		pro_image.style.height = "71px";
		pro_image.src = url;
		delete_picture.appendChild(pro_image);
		...
		question_infor.innerHTML = "您确定要删除<span style='color:red;'>《"+this.pro_title+"》</span>课题？";
	...
	ProjectInfor.prototype.deleteAction = function(projectModel , question_result){//将课题删除操作
		return function(){
			//使用Ajax技术异步的访问后台数据
			var url = "ProjectManagerServlet?type=project_recycle&project_id="+projectModel.pro_id;
		...
		return function(){
			if(deleteRequest.readyState == 1 || deleteRequest.readyState == 2 || deleteRequest.readyState == 3){
				...
				if(deleteRequest.status == 200){
					...
					if(result == "true"){
						//删除成功
						question_result.innerHTML = "课题删除成功！";
						
						//关闭提示框
						projectModel.cancelAction();
						
						//删除成功跳转到垃圾回收站
						//var url = ifIE ? "../ProjectManagerServlet?type=query_detail_project" : "ProjectManagerServlet?type=query_detail_project";
						var url = "SchoolUser/recycleProjects.jsp";
						window.open(url , "_self");
					}else{
						//删除失败
	...
	ProjectInfor.prototype.deleteProject = function(projectModel){//将该毕业课题放到回收站中
		return function(){
			if(DeleteWindow == null){
				...
				//设置标题信息
				var title = new Window_titleview(null , null, new Window_title("删除课题信息"), null, null, false, null, null, titleClose, null);
	...
	PagingFrame.prototype.createView = function(){
		...
		PagingProject.getPageNumberByCreateId($("teacher_id").value , null, $("proYear").value, function(pageParams){
	...
	PageProject.prototype.createPage = function(){
		try{
			if(this.pro_infor.length > 0){
				...
				promptWin.createConfirmView("<span style='color:red;'>Sorry 您还没有创建任何毕业课题！</span>" , "确定");	
			}
		}catch(error){
			alert(error);
		}
	...
	PageSlideObj.prototype.findProjects = function(pageNum , page){/* 查找页面对象信息使用DWR技术调用后台数据信息 */
		...
		/*  显示全部的分页数据信息 */
		PagingProject.pagingByCreateId(pageNum-1 , $("teacher_id").value, null, $("proYear").value, function(pageObj){
	...
	projectYearObj.prototype.clickYearAction = function(yearNum){
		return function(){
			var url = ifIE ? "../ProjectManagerServlet?type=enter_teacherPros&proYear="+yearNum : "ProjectManagerServlet?type=enter_teacherPros&proYear="+yearNum;
			window.open(url , "_self");
		}
	};
</script>
