...
<script type="text/javascript" src="dwr/interface/DWRStudentPaging.js"></script>
<style>
	*{
		margin: 0;
		padding: 0;
...
<script>
	...
	function init(){
		...
		pagingFrameObj = new PagingFrame($("find_project"));
	...
	ProjectInfor.prototype.showDetailProject = function(pro_id){//显示课题的详细信息内容(根据课题编号查询课题详细信息内容)
		return function(){
			var url = ifIE ? "../TeamManageServlet?type=enter_select_project&project_id="+pro_id : "TeamManageServlet?type=enter_select_project&project_id="+pro_id;
	 ...
	 */
	function PagingFrame(findButton){
		...
		this.queryProStage = "T";//需要查询的课题的状态类型
	}
	PagingFrame.prototype.createView = function(){
		//先获取分页参数信息然后再显示(这个是一个默认的分页对象-》》》这个后期将会对于搜索方式更改为学校审核通过的课题信息.
		DWRStudentPaging.getPageNumberBySchoolId($("student_id").value, this.queryFunType, this.queryProStage, function(pageParams){
	...
	PagingFrame.prototype.changePaging = function(queryFun , queryIndex){//实现根据不同的搜索方式实现分页queryIndex:查询索引
		switch(queryFun){
			case 1:{
				//这个是获取当前页面的分页数目
				DWRStudentPaging.getPageNumberBySchoolId($("student_id").value, $("school_id").value , this.queryProStage, function(pageParams){
		            if(pageParams.pagingNumbers == 0){//查询当前索引是否存在页面数据
			            ...
			            pro = new PageSlideObj(0 , pageParams.pagingNumbers);
						pro.initializeObj();
						pro.setPagingParams(pagingFrameObj.queryFunType , pagingFrameObj.queryProStage);//设置分页参数信息
						pro.findProPage(1);
					}
				});
				break;
			...
			case 2:{
				DWRStudentPaging.getPageNumberByCreateName($("student_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
			            ...
			            pro = new PageSlideObj(0 , pageParams.pagingNumbers);
						pro.initializeObj();
						pro.setPagingParams(pagingFrameObj.queryFunType , pagingFrameObj.queryProStage);
						pro.findProPage(1);
					}
				});
				break;
			...
			case 3:{
				DWRStudentPaging.getPageNumberByCollege($("student_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 4:{
				DWRStudentPaging.getPageNumberByProjectName($("student_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 5:{
				DWRStudentPaging.getPageNumberByPartNum($("student_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
			...
			case 6:{
				DWRStudentPaging.getPageNumberByHardNum($("student_id").value, $("school_id").value , queryIndex, this.queryProStage, function(pageParams){
					if(pageParams.pagingNumbers == 0){
						$("queryResult").style.color = "red";
		            	$("queryResult").innerHTML = "sorry 不存在你要找的课题信息...";
					}else{
	...
	PagingFrame.prototype.createQueryWinodw = function(){
		...
		selectText.innerHTML = "<span style='float:left'>关键字：</span><div class='input_frame'><input type='text' id='query_Key'/></div>";
	...
	PagingFrame.prototype.closeWinodw = function(parentObj){//重写关闭事件
		return function(){  
			...
			pagingFrameObj.queryProStage = "T";
		};
	};
	...
	PageProject.prototype.createPage = function(){
		try{
			if(this.pro_infor.length > 0){
				...
				promptWin.createConfirmView("<span style='color:red;'>当前系统还未到选题时间！<span style='color:#aaaaaa;font-weight:normal;'>(可咨询教务处)</span></span>" , "确定");	
			}
		}catch(error){
			alert(error);
		}
	...
	function PageSlideObj(pageIndex , allPageNum){
		...
		this.pagingProStage = "T";//当前需要查询的页面的课题状态类型（默认为S）
	}
	...
	PageSlideObj.prototype.setPagingParams = function(pagingFunType , pagingProStage){//设置查询方式索引
	...
	PageSlideObj.prototype.findProjects = function(pageNum , page){/* 查找页面对象信息使用DWR技术调用后台数据信息 */
		...
		switch(this.pagingFunType){
			case 1:{
				DWRStudentPaging.pagingBySchoolId(pageNum-1 , $("student_id").value, $("school_id").value, this.pagingProStage, function(pageObj){
			...
			case 2:{
				DWRStudentPaging.pagingByCreateName(pageNum-1 , $("student_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 3:{
				DWRStudentPaging.pagingByCollege(pageNum-1 , $("student_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 4:{
				DWRStudentPaging.pagingByProjectName(pageNum-1 , $("student_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 5:{
				DWRStudentPaging.pagingByPartNum(pageNum-1 , $("student_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
			...
			case 6:{
				DWRStudentPaging.pagingByHardNum(pageNum-1 , $("student_id").value, $("school_id").value, $("query_Key").value, this.pagingProStage, function(pageObj){
					for(var i=0;i<pageObj.length;i++){
						var pro = new ProjectInfor(pageObj[i].projectName, pageObj[i].id, pageObj[i].schoolId, pageObj[i].picture, pageObj[i].createrName, pageObj[i].createrId,
									 pageObj[i].partNum, pageObj[i].college, pageObj[i].workload, pageObj[i].hardNum, pageObj[i].projectFile, pageObj[i].stage);
