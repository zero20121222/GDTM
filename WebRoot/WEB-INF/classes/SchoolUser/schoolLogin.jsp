...
<title>青岛工学院登录入口</title>
<link rel="icon" href="images/favicon.ico" type="image/x-icon"></link>
<link href="css/school_user_login.css" rel="stylesheet" type="text/css"></link>
...
function register_NameEvent(){
	...
	user_name.value = cookieObj.getCookie("GDT_School_Name");
...
function register_Submit(){
	...
	inter_submit.onclick = function(){
		if($("user_name").value == ""){
			...
			if(!waiting_login){
				var url = "SchoolLoginServlet?type=Login_Check&user_name="+$("user_name").value+"&user_password="+$("user_password").value+"&user_code="+$("user_code").value;
	...
	if(request.readyState == 1 || request.readyState == 2 || request.readyState == 3){
		...
		if(request.status == 200){//成功
			...
			if(user_id > 0){
				...
				cookieObj.setCookie("GDT_School_Name" , $("user_name").value, 7);
				...
				//解决window.open对于浏览器兼容性问题（相对路径的选取问题）
				var login_url = ifIE ? "../SchoolLoginServlet?type=Login_main" : "SchoolLoginServlet?type=Login_main"
...
<div class="login_body">
	<div id="body_frame">
	  <div class="frame_left">
			...
			<div>为教师提供 1.课题创建平台2.课题管理功能</div>
			<div>为学生提供 1.课题选择平台2.团队创建3.课题管理</div>
