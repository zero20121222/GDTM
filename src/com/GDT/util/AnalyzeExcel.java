...
public class AnalyzeExcel {
    ...
    public <T> List<T> analyzeURL(String fileUrl, int rowStart, AnalyzeAction<T> action){
        ...
        try{
            ...
            log.debug("analyze excel file used "+(System.currentTimeMillis() - start));
        }catch(IOException e){
            log.error("analyze excel file failed, error code={}", e);
        }finally {
            try {
                inputStream.close();
            }catch(Exception e){
                log.error("close file failed, error code={}", e);
            }
        }
